...
| `service`   | string | `control-plane` \| `modal-infra` \| `slack-bot` \| `discord-bot` |
...
| --------------------- | ----------------- | ------------------------ |
| `trace_id`            | Cross-service     | Router / bot edge        |
...
A prompt run flows: **bot → control-plane → modal-infra → sandbox**.
